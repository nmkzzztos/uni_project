...
class Game:
    ...
    def _get_player_move(self) -> [str, tuple[int, int]]:
        """
    ...
    def _handle_play_mode(self, entity: Entity) -> tuple[int, int]:
        """
        Handle play mode (print board and entity information, get player move).

        Args:
            entity (Entity): The entity whose turn it is.
        """
        self.board.print_board(entity, state=self.state)
        print(entity)
        for unit in entity.units:
            print(unit)
        split, (dx, dy) = self._get_player_move()
        new_unit = entity.split(self.board_size) if split == "y" else None
        self.entities.add_player(new_unit) if new_unit else None
        return (dx, dy)

    def _show_target_and_content(self, entity: Entity, dx: int, dy: int) -> None:
        target_position = Position(
            (entity.position.row_index - dy) % self.board_size,
            (entity.position.column_index + dx) % self.board_size,
        )
