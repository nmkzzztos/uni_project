...
class Player(MoveableEntity):
    def __init__(self, position: Position, energy: int):
        ...
        self.copies: List[Player] = []

    def split(self, player: Entity):
        if self.energy >= 40:
            self.energy -= 20
            random_position = Position(
                player.position.row_index + random.randint(-1, 1),
                player.position.column_index + random.randint(-1, 1),
            )
            new_copy = Player(random_position, 20)
