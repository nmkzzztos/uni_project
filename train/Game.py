...
class Game:
    ...
    def __init__(
        ...
        play: bool = False,
        path_for_player: str = "test_food_actions.pkl",
        path_for_beast: str = "tables/main.pkl",
    ) -> None:
    ...
    def _game_turn(
        ...
        for entity in all_entities:
            ...
            self.board.add_food(self.entities)
            ...
            if main == entity:
                ...
                # Print board and entity information for save mode
                    ...
                    if entity.energy >= 40:
                        new_entity = entity.split(entity)
                        self.entities.add_player(new_entity)
            ...
            if entity == main:
                diff_energy = entity.energy

            self._update_energy(dx, dy, entity)

            if entity == main:
                diff_energy = entity.energy - diff_energy

            self.board.clear_entity_at(entity.position)

            entity.move(dx, dy, self.board_size)

        return (dx_player, dy_player, state_return, content, diff_energy)
    ...
    def start(self) -> None:
        ...
        while main_player.energy >= 1:
            ...
            # Run a game turn (move entities, update energy, etc.)
            dx, dy, state, content, diff_energy = self._game_turn(
                all_entities, main_player
            )

            # Update Q-tables if in save mode
            if self.save_mode and main_player.energy != 0 and content != None:
                reward = self.q_for_player.get_reward(content, False, diff_energy)
        ...
        if game_won == False:
            if self.save_mode and main_player.energy != 0:
                reward = self.q_for_player.get_reward(content, True, diff_energy)
                self.q_for_player.update_q_value(state, (dx, dy), reward)
